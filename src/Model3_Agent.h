...
#ifndef MLPACK_METHODS_RL_ENVIRONMENT_MYMODEL3_HPP
#define MLPACK_METHODS_RL_ENVIRONMENT_MYMODEL3_HPP
...
public:
    ...
    public:
        ...
        double MoveDirection() const { return data[26]; }
        double& MoveDirection() { return data[26]; }

        //*******2dir
        //@BIAS
        double Bias() const { return data[27]; }
        double& Bias() { return data[27]; }
    ...
    public:
        enum actions
        {
            ...
            rotate_backward_joint9,     //joint9
            stop_joint9,
            rotate_forward_joint9,
            rotate_backward_joint10,    //joint10
            stop_joint10,
            rotate_forward_joint10,
            rotate_backward_joint11,    //joint11
            stop_joint11,
            rotate_forward_joint11,
            no_action
        };
    ...
    public:
        ...
        ContiniousAction() : action(Model3_Agent::ContiniousAction::size) {}
    };
    Model3_Agent(PhysicsWorld* training_world,
        SimWindow* training_window,
        int output_type,
        const size_t maxsteps = 100,
        bool dir_train = false,
        bool write_twoway = false,
        const double thetaThresholdRadians = 1.6,
        ...
        State& nextState)
    {
        ...
        {
            if (action_type == 0)
                if (training_world->obj3_joints[joint_num]->GetMotorSpeed() > -4.0)
                    training_world->obj3_joints[joint_num]->SetMotorSpeed(-2.0f);
                    //training_world->obj3_joints[joint_num]->SetMotorSpeed(training_world->obj3_joints[joint_num]->GetMotorSpeed() - DISCRETE_SPPED_GAIN);
            if (action_type == 1)
                training_world->obj3_joints[joint_num]->SetMotorSpeed(0.0f);
            if (action_type == 2)
                if (training_world->obj3_joints[joint_num]->GetMotorSpeed() < 4.0)
                    training_world->obj3_joints[joint_num]->SetMotorSpeed(2.0f);
                    //training_world->obj3_joints[joint_num]->SetMotorSpeed(training_world->obj3_joints[joint_num]->GetMotorSpeed() + DISCRETE_SPPED_GAIN);
        ...
        //fill next step data
        nextState.LinearVelocity() = (double)training_world->obj3_parts[6]->GetLinearVelocity().x;
        nextState.verticalVelocity() = (double)training_world->obj3_parts[6]->GetLinearVelocity().y;
        for (int i = 0; i < training_world->obj3_num_parts; i++)
        {
            nextState.Angle(i) = (double)training_world->obj3_parts[i]->GetAngle();
            nextState.AngularVelocity(i) = (double)training_world->obj3_parts[i]->GetAngularVelocity();
        }
        ...
        if (training_window->window_pause_flag == false)
        {
            ...
            training_window->SetOffset(training_world->obj3_parts[6]);
        ...
        if (perform_dir_train) {
            if (randomness[0] == (int)stepsPerformed) {
                ...
                dir_change_pos = training_world->obj3_parts[6]->GetPosition().x;
            }
        ...
        if (state.MoveDirection() == 0.0) {
            ...
            if ((nextState.LinearVelocity() > 0.05 && nextState.MoveDirection() == 1.0) ||
                ...
                reward = 1.0;
            if (((training_world->obj3_parts[6]->GetPosition().x - dir_change_pos) > final_distance && nextState.MoveDirection() == 1.0) ||
                ((training_world->obj3_parts[6]->GetPosition().x - dir_change_pos) < -final_distance && nextState.MoveDirection() == -1.0))
                reward = 5.0;
        }
        ...
        State& nextState)
    {
        ...
        {
            if (output_type == 2) //ga
                training_world->obj3_joints[i]->SetMotorSpeed((float)action.action[i]);
            if (output_type == 1) //neat
            {
                if ((((action.action[i] - 0.5) > 0.0) && training_world->obj3_joints[i]->GetMotorSpeed() < 4.0f) ||
                    (((action.action[i] - 0.5) < 0.0) && training_world->obj3_joints[i]->GetMotorSpeed() > -4.0f))
                    training_world->obj3_joints[i]->SetMotorSpeed(training_world->obj3_joints[i]->GetMotorSpeed() + ((float)action.action[i] - 0.5f));
                training_world->obj3_joints[i]->SetMaxMotorTorque(std::abs((float)action.action[i] - 0.5f) * 600.f + 300.f);
            }
        }
        //simlate one training_world step
        training_world->CheckAngleLimits();
        ...
        //return training_world->obj3_parts[6]->GetPosition().x * 500.0 + 50.0;      //reward based on final destination reached
        //************2dir
        ...
        if (output_type == 1)
        {
            ...
            if (perform_dir_train) {
                if (randomness[0] == (int)stepsPerformed || randomness[1] == (int)stepsPerformed) {
                    if (randomness[0] == (int)stepsPerformed)
                        ...
                        nextState.MoveDirection() = (double)randomness[3];
                    if (state.MoveDirection() != 0.0)
                        fit_in_dir_change += state.MoveDirection() * ((training_world->obj3_parts[6]->GetPosition().x - dir_change_pos) * 500.0);
                    else
                        fit_in_dir_change += -std::abs((training_world->obj3_parts[6]->GetPosition().x - dir_change_pos) * 500.0);
                    dir_change_pos = training_world->obj3_parts[6]->GetPosition().x;
            ...
            if (nextState.MoveDirection() != 0.0)
                return_val = fit_in_dir_change + (nextState.MoveDirection()
                    * ((training_world->obj3_parts[6]->GetPosition().x - dir_change_pos) * 500.0));
            else
                return_val = fit_in_dir_change + (-std::abs((training_world->obj3_parts[6]->GetPosition().x - dir_change_pos) * 500.0));
            // ----------------------------------------------------------------
        }
        else
            return_val = (double)(training_world->obj3_parts[6]->GetPosition().x * 500.f + 50.f);
    ...
    State InitialSample()
    {
        ...
        stepsPerformed = 0;
        fitness = 0.0;
        //duble res to be sure cuz that human robot wiggle as fuck
        //training_world->resetMechanism();
        training_world->resetMechanism();

        initial_data.LinearVelocity() = (double)training_world->obj3_parts[6]->GetLinearVelocity().x;
        initial_data.verticalVelocity() = (double)training_world->obj3_parts[6]->GetLinearVelocity().y;
        for (int i = 0; i < training_world->obj3_num_parts; i++)
        {
            initial_data.Angle(i) = (double)training_world->obj3_parts[i]->GetAngle();
            initial_data.AngularVelocity(i) = (double)training_world->obj3_parts[i]->GetAngularVelocity();
        }
    ...
    bool IsTerminal(const State& state) const
    {
        ...
        if (std::abs(state.Angle(6)) > thetaThresholdRadians)
            return true;
        if (training_world->obj3_parts[6]->GetPosition().y < 0.8)
            return true;
        if (std::abs(state.Angle(4) - state.Angle(5)) > 2.7)
            return true;
