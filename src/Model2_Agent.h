...
#ifndef MLPACK_METHODS_RL_ENVIRONMENT_MYMODEL2_HPP
#define MLPACK_METHODS_RL_ENVIRONMENT_MYMODEL2_HPP
...
public:
	...
	public:
      ...
      /*  double MoveDirectionleft() const { return data[13]; }
        double& MoveDirectionleft() { return data[13]; }*/
    ...
    public:
        ...
        ContiniousAction() : action(Model2_Agent::ContiniousAction::size) {}
    };
    Model2_Agent(PhysicsWorld* training_world,
        ...
        State& nextState)
    {
        ...
        {
            if (action_type == 0)
                if (training_world->obj2_joints[joint_num]->GetMotorSpeed() > -4.0)
                    training_world->obj2_joints[joint_num]->SetMotorSpeed(-2.0f);
                    //training_world->obj2_joints[joint_num]->SetMotorSpeed(training_world->obj2_joints[joint_num]->GetMotorSpeed() - DISCRETE_SPPED_GAIN);
            if (action_type == 1)
                training_world->obj2_joints[joint_num]->SetMotorSpeed(0.0f);
            if (action_type == 2)
                if (training_world->obj2_joints[joint_num]->GetMotorSpeed() < 4.0)
                    training_world->obj2_joints[joint_num]->SetMotorSpeed(2.0f);
                    //training_world->obj2_joints[joint_num]->SetMotorSpeed(training_world->obj2_joints[joint_num]->GetMotorSpeed() + DISCRETE_SPPED_GAIN);
        ...
        //fill next step data
        nextState.LinearVelocity() = (double)training_world->obj2_parts[4]->GetLinearVelocity().x;
        nextState.verticalVelocity() = (double)training_world->obj2_parts[4]->GetLinearVelocity().y;
        for (int i = 0; i < training_world->obj2_num_parts; i++)
        {
            nextState.Angle(i) = (double)training_world->obj2_parts[i]->GetAngle();
            nextState.AngularVelocity(i) = (double)training_world->obj2_parts[i]->GetAngularVelocity();
        }
        nextState.MoveDirection() = state.MoveDirection();
        //nextState.MoveDirectionleft() = state.MoveDirectionleft();
        ...
        if (training_window->window_pause_flag == false)
        {
            ...
            training_window->SetOffset(training_world->obj2_parts[4]);
        ...
        if (perform_dir_train) {
            if (randomness[0] == (int)stepsPerformed) {
                ...
                dir_change_pos = training_world->obj2_parts[4]->GetPosition().x;
            }
        ...
        if (state.MoveDirection() == 0.0) {
            ...
            if ((nextState.LinearVelocity() > 0.05 && nextState.MoveDirection() == 1.0) ||
                ...
                reward = 1.0;
            if (((training_world->obj2_parts[4]->GetPosition().x - dir_change_pos) > final_distance && nextState.MoveDirection() == 1.0) ||
                ((training_world->obj2_parts[4]->GetPosition().x - dir_change_pos) < -final_distance && nextState.MoveDirection() == -1.0))
                reward = 5.0;
        }
        ...
        State& nextState)
    {
        ...
        {
            if (output_type == 2) //ga
                training_world->obj2_joints[i]->SetMotorSpeed((float)action.action[i]);
            if (output_type == 1) //neat
            {
                if ((((action.action[i] - 0.5) > 0.0) && training_world->obj2_joints[i]->GetMotorSpeed() < 4.0f) ||
                    (((action.action[i] - 0.5) < 0.0) && training_world->obj2_joints[i]->GetMotorSpeed() > -4.0f))
                    training_world->obj2_joints[i]->SetMotorSpeed(training_world->obj2_joints[i]->GetMotorSpeed() + ((float)action.action[i] - 0.5f));
                training_world->obj2_joints[i]->SetMaxMotorTorque(std::abs((float)action.action[i] - 0.5f) * 100.f + 50.f);
            }
        }
        //simlate one training_world step
        training_world->CheckAngleLimits();
        ...
        //return training_world->obj2_parts[4]->GetPosition().x * 500.0 + 50.0;      //reward based on final destination reached
        //**********2dir
        ...
        if (output_type == 1)
        {
            ...
            if (perform_dir_train) {
                if (randomness[0] == (int)stepsPerformed || randomness[1] == (int)stepsPerformed) {
                    if (randomness[0] == (int)stepsPerformed)
                        ...
                        nextState.MoveDirection() = (double)randomness[3];
                    if (state.MoveDirection() != 0.0)
                        fit_in_dir_change += state.MoveDirection() * ((training_world->obj2_parts[4]->GetPosition().x - dir_change_pos) * 500.0);
                    else
                        fit_in_dir_change += -std::abs((training_world->obj2_parts[4]->GetPosition().x - dir_change_pos) * 500.0);
                    dir_change_pos = training_world->obj2_parts[4]->GetPosition().x;
            ...
            if (nextState.MoveDirection() != 0.0)
                return_val = fit_in_dir_change + (nextState.MoveDirection()
                    * ((training_world->obj2_parts[4]->GetPosition().x - dir_change_pos) * 500.0));
            else
                return_val = fit_in_dir_change + (-std::abs((training_world->obj2_parts[4]->GetPosition().x - dir_change_pos) * 500.0));
            // ----------------------------------------------------------------
        }
        else
            return_val = (double)(training_world->obj2_parts[4]->GetPosition().x * 500.f + 50.f);
    ...
    State InitialSample()
    {
        ...
        initial_data.LinearVelocity() = (double)training_world->obj2_parts[4]->GetLinearVelocity().x;
        initial_data.verticalVelocity() = (double)training_world->obj2_parts[4]->GetLinearVelocity().y;
        for (int i = 0; i < training_world->obj2_num_parts; i++)
        {
            initial_data.Angle(i) = (double)training_world->obj2_parts[i]->GetAngle();
            initial_data.AngularVelocity(i) = (double)training_world->obj2_parts[i]->GetAngularVelocity();
        }
        ...
        if (perform_dir_train)
        {
            ...
            initial_data.MoveDirection() = 1.0;
        //initial_data.MoveDirectionleft() = 0.0;
        //@BIAS
    ...
    bool IsTerminal(const State& state) const
    {
        ...
        if (std::abs(state.Angle(4)) > thetaThresholdRadians)
            return true;
        if (training_world->obj2_parts[4]->GetWorldPoint(b2Vec2(-0.39f, -0.19f)).y < 0.25 
            || training_world->obj2_parts[4]->GetWorldPoint(b2Vec2(0.39f, -0.19f)).y < 0.25)
            return true;
