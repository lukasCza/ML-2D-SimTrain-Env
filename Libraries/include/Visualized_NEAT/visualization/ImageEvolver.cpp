...
#include <Visualized_NEAT/visualization/ImageEvolver.h>

#include <Visualized_NEAT/neat/UtilFuncs.h>

#include <iostream>

void ImageEvolver::train(const neat::NetworkGenotype &genotype, size_t populationSize, size_t numGenerations, std::mt19937 &generator) {
	...
	std::uniform_real_distribution<float> distPositionPerturbation(-maxMutationPerturbationPosition, maxMutationPerturbationPosition);

	const size_t numNodes = genotype.getNumHidden() + genotype.getNumInputs() + genotype.getNumOutputs();
	...
	if (!genotype.getNodeData(i)._connections.empty()) { // Node is in use (has connections)
		_activeNodeIndexToNodeIndex.push_back(i);
		_nodeIndexToActiveNodeIndex.push_back(_activeNodeIndexToNodeIndex.size() - 1);
	}
	else
	...
	std::uniform_int_distribution<int> distActiveNodes(0, _activeNodeIndexToNodeIndex.size() - 1);
	...
	for (size_t g = 0; g < numGenerations; g++) {
		...
		for (size_t i = 0; i < populationSize; i++) {
			...
			for (size_t j = 0; j < numActiveNodes; j++) {
				...
				neat::NetworkGenotype::NodeData nodeData = genotype.getNodeData(_activeNodeIndexToNodeIndex[j]);

				size_t numConnectionsForNode = nodeData._connections.size();

				for (std::list<std::shared_ptr<neat::ConnectionGene>>::iterator it = nodeData._connections.begin(); it != nodeData._connections.end(); it++) {
					assert((*it)->_inIndex != -1);

					if ((*it)->_inIndex == j)
						continue;

					const size_t inIndex = _nodeIndexToActiveNodeIndex[(*it)->_inIndex];

					// Add difference from a distance of 1 between connected node as cost
					float distanceCost = pow(std::abs(normalDistance - distance(population[i]._positions[j], population[i]._positions[inIndex])), normalDistanceExp);

					directionSum += normalize(population[i]._positions[inIndex] - population[i]._positions[j]);

					fitnessAndIndex._fitness -= normalDistanceInfluence * distanceCost;
				}

				// Add all position costs
				for (size_t k = 0; k < numActiveNodes; k++)
					fitnessAndIndex._fitness -= unconnectedNodeDistanceInfluence * pow(std::abs(unconnectedNodeDistance - distance(population[i]._positions[j], population[i]._positions[k])), unconnectedNodeDistanceExp);
			}

			if (directionSum.x != 0.0f || directionSum.y != 0.0f)
				fitnessAndIndex._fitness -= unevenConnectionAnglesPunishment * powf(magnitude(directionSum), unevenConnectionAnglesExp);
		...
		const int numReproduce = std::max(2, static_cast<int>(reproduceRatio * static_cast<float>(populationSize) + 0.5f));
		...
		for (int i = 0; i < numReproduce; i++) {
			...
			if (i == 0) // Most fit one in order to keep track of best across all generations
		...
		// Randomly repoduce top performers until have a large enough population again
		std::vector<CandidateSolution> newPopulation(populationSize);

		std::uniform_int_distribution<int> distNumReproduce(0, numReproduce - 1);

		for (size_t i = 0; i < populationSize; i++) {
			// Get 2 random members of best group
			CandidateSolution &solution1 = population[bestIndices[distNumReproduce(generator)]];
			CandidateSolution &solution2 = population[bestIndices[distNumReproduce(generator)]];
			...
			for (; crossIndex < crossoverPoint; crossIndex++)
				childSolution._positions[crossIndex] = solution1._positions[crossIndex];
			...
			if (dist01(generator) < mutationRate) {
				childSolution._positions[j].x += distPositionPerturbation(generator);
				childSolution._positions[j].y += distPositionPerturbation(generator);
			}
